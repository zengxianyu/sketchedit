...
with open(opt.load_opt_face, "rb") as f:
    opt_face = pickle.load(f)
with open(opt.load_opt_img, "rb") as f:
    opt_img = pickle.load(f)

model_face = models.create_model(opt_face)
model_face.eval()
...
def get_result(model, img, mask):
    ...
    return result, mask_gen

def process_image(img, mask, name, opt, save_to_input=True):
    img =img.convert("RGB")
    image0 = np.array(img)
    result, _ = get_result(model_img, img, mask)
    img_face, mask_face, affine = get_face_image(img, mask)
    if affine is not None:
        result_face, mask_gen_face = get_result(model_face, img_face, mask_face)
        result = warpback_image(image0, result, result_face, mask_gen_face, affine)
...
def warpback_image(image0, image1, image, mask_gen, affine):
    h,w,c = image0.shape
    R, T, s = affine
    rot_mat = np.concatenate((R/s,-R/s@T[...,None]),1)
    mask_sq = np.ones_like(image)
    mask_sq = cv2.warpAffine(mask_sq, rot_mat, (w,h))
    image = cv2.warpAffine(image, rot_mat, (w,h))
    mask = cv2.warpAffine(mask_gen, rot_mat, (w,h))
    mask = mask[...,None]
    kernel = np.ones((3, 3), np.uint8)
    mask_sq = cv2.erode(mask_sq, kernel)
    image0 = image0*mask_sq+image1*(1-mask_sq)
    #lms_rot = lms@R.T+T
